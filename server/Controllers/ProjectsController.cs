...
public record ProjectDto(int Id, string Title, string Description, string? RepoUrl, string? LiveUrl, string? ImageUrl, List<string> Skills);
        ...
        try
        {
            ...
            var projects = await _dbContext.Projects
                ...
                .Select(p => new ProjectDto(
                    ...
                    p.LiveUrl,
                    p.ImageUrl,
                    p.ProjectSkills.Select(ps => ps.Skill.Name).ToList()
                ))
                .Skip(skip)
                .Take(pageSize)
                .ToListAsync();
